/**
 * Script Migrasi Data Productivity Daily Reference (Earned Hours) dari
 * project lama (database-nu-two) ke PKIS-PLUS
 *
 * Sumber : tabel `productivity_daily_reference` (database-nu-two) - kolom: tanggal, eh_jam, wh_jam
 * Tujuan : tabel `productivity_daily_reference` (PKIS-PLUS)      - kolom: id, tanggal, eh_jam, is_active, created_at
 *
 * Catatan: kolom `wh_jam` di sumber TIDAK ADA di tabel tujuan (dan tidak
 * dipakai di manapun di kode PKIS-PLUS), jadi kolom ini di-skip / tidak ikut
 * dimigrasikan. Kolom `id` juga tidak di-carry dari sumber (sumber tidak
 * punya kolom id) - dibiarkan generate otomatis oleh Supabase.
 *
 * Penggunaan:
 *   node scripts/import-productivity-daily-reference.js --dry-run
 *   node scripts/import-productivity-daily-reference.js
 *   node scripts/import-productivity-daily-reference.js --file=scripts/data/productivity_daily_reference_rows.csv --dry-run
 */
...
async function main() {
  ...
  const filePath = customFileArg
    ...
    : path.resolve(process.cwd(), 'scripts/data/productivity_daily_reference_rows.csv');

  console.log('='.repeat(65));
  console.log('📦 PKIS-PLUS: SCRIPT MIGRASI PRODUCTIVITY DAILY REFERENCE (EH)');
  ...
  console.log('ℹ️  Kolom "wh_jam" di CSV sumber akan DI-SKIP (tidak ada di tabel tujuan).\n');
  ...
  for (let idx = 0; idx < rawRows.length; idx++) {
    ...
    if (!tanggal) {
      skippedRecords.push({ rowNum, tanggal, reason: 'tanggal kosong' });
      continue;
    }

    validRecords.push({
      tanggal,
      eh_jam: parseNumeric(row.eh_jam, 0),
      is_active: true,
    });
  }
  ...
  if (skippedRecords.length > 0) {
    ...
    skippedRecords.forEach((s) => {
      console.log(`   - [Baris ${s.rowNum}] tanggal: '${s.tanggal}' -> Alasan: ${s.reason}`);
    });
  }
  ...
  for (let i = 0; i < validRecords.length; i += BATCH_SIZE) {
    ...
    const { error } = await supabase
      ...
      .upsert(batch, { onConflict: 'tanggal' });
