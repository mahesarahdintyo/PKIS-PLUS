/**
 * Script Migrasi Data Downtime Problems dari project lama (database-nu-two) ke PKIS-PLUS
 *
 * Sumber : tabel `downtime_problems` (database-nu-two)
 * Tujuan : tabel `prod_downtime_problems` (PKIS-PLUS)
 *
 * Penggunaan:
 *   node scripts/import-downtime-problems.js --dry-run
 *   node scripts/import-downtime-problems.js
 *   node scripts/import-downtime-problems.js --file=scripts/data/downtime_problems_rows.csv --dry-run
 */
...
async function main() {
  ...
  const filePath = customFileArg
    ...
    : path.resolve(process.cwd(), 'scripts/data/downtime_problems_rows.csv');

  console.log('='.repeat(65));
  console.log('📦 PKIS-PLUS: SCRIPT MIGRASI DOWNTIME PROBLEMS');
  ...
  const seen = new Set(); // dedup (mesin,value) - CSV riwayat panjang, jaga-jaga ada duplikat
  ...
  for (let idx = 0; idx < rawRows.length; idx++) {
    ...
    if (!value) {
      skippedRecords.push({ rowNum, mesin, value: '(kosong)', reason: 'Nilai (value) kosong' });
      continue;
    }
    if (!mesin || !VALID_MACHINES.includes(mesin)) {
      skippedRecords.push({ rowNum, mesin, value, reason: `Mesin '${mesin}' tidak valid` });
      continue;
    }

    const dedupKey = `${mesin}::${value.toLowerCase()}`;
    if (seen.has(dedupKey)) {
      skippedRecords.push({ rowNum, mesin, value, reason: 'Duplikat (mesin+value sama sudah ada di baris sebelumnya)' });
      continue;
    }
    seen.add(dedupKey);
  ...
  if (skippedRecords.length > 0) {
    ...
    skippedRecords.forEach((s) => {
      console.log(`   - [Baris ${s.rowNum}] mesin: '${s.mesin}', value: '${s.value}' -> Alasan: ${s.reason}`);
    });
  }
