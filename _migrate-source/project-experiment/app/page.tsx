...
import { registerInternalVizPlugins } from "@/lib/chartPlugins";
...
export default function DashboardPage() {
  ...
  // Jumlah line yang aktif produksi (stroke > 0) periode ini.
  const [lineAktif, setLineAktif] = useState(0);
  // Achievement % (qty aktual vs qty rencana planning), gabungan semua line.
  ...
  const bounds = useCallback(() => {
    // tahunan/bulanan pakai tahunPilih (selector tahun terpisah dari date-picker "tanggal"
    // yang cuma dipakai di mode harian) supaya ganti tahun/bulan tidak diam-diam balik
    // ke tahun dari tanggal harian yang mungkin beda.
  ...
  /* ── Downtime drill-down modal (port dari openDowntimeDetail() index.html) ── */
  const openDowntimeDetail = useCallback(async (filters: { mesin?: string; mesinLabel?: string; kategori?: string }) => {
    ...
    let q = supabase.from("downtime_log").select("*")
  ...
  /* ── fetchLineTrend (port from vanilla) ─────────────── */
  const fetchLineTrend = useCallback(async () => {
    if (periodMode === "harian") {
      // per-jam handled by gsph_hourly RPC in fetchDashboardData
    ...
    try {
      const res = await Promise.all(MACHINES.map((m) =>
        supabase.rpc("gsph_trend_bucketed", {
  ...
  /* ── fetchProductivityTrend & fetchProductivityToday (port from index.html) ── */
  const fetchProductivityTrend = useCallback(async () => {
    ...
    try {
      const { data } = await supabase.rpc("productivity_trend_bucketed", {
  ...
  /* ── fetchMiniTrend (port from vanilla) ─────────────── */
  const fetchMiniTrend = useCallback(async () => {
    ...
    try {
      const perPeriod = await Promise.all(periods.map(async (p) => {
        ...
        // stroke/ng/ng_value dihitung server-side lewat performance_aggregate per mesin
        // (production_log tidak punya kolom ng_qty/ok_qty/ng_value) — port dari index.html.
        const safetyRpcP  = Promise.resolve(supabase.rpc("safety_summary",         { p_start: sDate, p_end: eDate })).catch(() => ({ data: null, error: true }));
        const attRpcP     = Promise.resolve(supabase.rpc("attendance_summary",      { p_start: sDate, p_end: eDate })).catch(() => ({ data: null, error: true }));
        const scrapRpcP   = Promise.resolve(supabase.rpc("scrap_top_end_summary",   { p_start: sDate, p_end: eDate })).catch(() => ({ data: null, error: true }));
        const aggAllP     = Promise.all(MACHINES.map((m) =>
          supabase.rpc("performance_aggregate", { p_mesin: m.key, p_stasiun_list: null, p_start: sIso, p_end: eIso })
        ));
  ...
  /* ── fetchAttendanceByShift (port from vanilla) ──────── */
  // Mengembalikan hasil (bukan langsung setState) supaya fetchDashboardData bisa
  // menerapkan semua state sekaligus di akhir, sesudah cek stale-request.
  const fetchAttendanceByShift = useCallback(async (startDate: string, endDate: string) => {
    try {
      const [r1, r2] = await Promise.all([
        supabase.rpc("attendance_summary", { p_start: startDate, p_end: endDate, p_shift: "1" }),
        supabase.rpc("attendance_summary", { p_start: startDate, p_end: endDate, p_shift: "2" }),
      ]);
  ...
  /* ── Fetch ───────────────────────────────────────────── */
  // isCancelled: dicek sebelum menerapkan hasil ke state — kalau periodMode/tanggal
  // berganti lagi sebelum fetch ini selesai, hasil basi ini dibuang begitu saja
  // supaya tidak menimpa state dari request yang lebih baru (lihat efek di bawah).
  const fetchDashboardData = useCallback(async (isCancelled: () => boolean = () => false) => {
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (profData) profileResult = profData as Profile;
      }
      ...
      // production_log/downtime_log tidak punya kolom "tanggal"/"shift" — waktu difilter
      // lewat waktu_awal (timestamptz), dan shift 1/2 direpresentasikan sebagai rentang jam
      // lewat bounds() (shift 1: 07:00-19:30, shift 2: 19:30-07:00 keesokan hari).
      const { start: rangeStart, end: rangeEnd } = bounds();
      const rangeStartIso = rangeStart.toISOString();
      const rangeEndIso = rangeEnd.toISOString();

      /* Per-mesin: stroke/ng/ng_value/downtime/dandori/wh_menit lewat RPC performance_aggregate
         (port dari fetchAll() index.html) — bukan dihitung ulang dari raw production_log/
         downtime_log yang skemanya tidak match (production_log tidak punya ok_qty/ng_qty,
         downtime_log tidak punya durasi_menit). Target GSPH per mesin murni dari
         mesin_settings — kalau belum diset, targetGsph = 0 (tidak ada fallback). */
      const [aggResults, topProbResults, byCatResults, settingsRes] = await Promise.all([
        Promise.all(MACHINES.map((m) =>
          supabase.rpc("performance_aggregate", { p_mesin: m.key, p_stasiun_list: null, p_start: rangeStartIso, p_end: rangeEndIso })
        )),
        Promise.all(MACHINES.map((m) =>
          supabase.rpc("downtime_top_problems", { p_mesin: m.key, p_stasiun_list: null, p_start: rangeStartIso, p_end: rangeEndIso, p_limit: 10 })
        )),
        Promise.all(MACHINES.map((m) =>
          supabase.rpc("downtime_by_category", { p_mesin: m.key, p_stasiun_list: null, p_start: rangeStartIso, p_end: rangeEndIso })
        )),
        supabase.from("mesin_settings").select("*"),
      ]);
      ...
      let targetGsphSum = 0, targetGsphCount = 0; // untuk rata-rata fleet (hanya mesin yg punya target eksplisit)
      ...
      MACHINES.forEach((m, idx) => {
        ...
        // Target eksplisit dari mesin_settings — kalau belum diset, targetGsph = 0
        // sehingga performanceFactor mesin itu tetap 0% (tidak ada fallback hardcode).
      ...
      // Totals gabungan — dijumlahkan dari tiap mesin (whJam disum, bukan konstanta
      // workMins), lalu availability/performance/quality/OEE fleet dihitung dari hasil sum itu.
      // targetGsph fleet = rata-rata hanya dari mesin yang punya target eksplisit (targetGsph > 0).
      ...
      try {
        const achResults = await Promise.all(MACHINES.map((m) =>
          supabase.rpc("achievement_aggregate", { p_mesin: m.key, p_start: rangeStartIso, p_end: rangeEndIso })
        ));
      ...
      // Fleet Top 10 & Pareto — sumber downtime_top_problems per mesin (durasi sudah
      // dihitung di SQL dari waktu_akhir - waktu_awal), bukan dari raw downtime_log.
      ...
      const paretoResult = Object.entries(probAgg)
        ...
        .slice(0, 6);
      // Lebar bar diskalakan ke nilai terbesar yang ditampilkan (bukan ke total
      // downtime pabrik), supaya batang paling besar mengisi penuh track — persentase
      // di label tetap "% dari total" seperti biasa.
      const maxParetoMenit = Math.max(1, ...paretoResult.map((r) => r.menit));
      paretoResult.forEach((r: any) => { r.barPct = (r.menit / maxParetoMenit) * 100; });

      // Worst-5 downtime per mesin — sumber sama dengan fleetTop10 (topProbResults),
      // sudah sorted DESC oleh RPC, ambil 5 teratas per mesin.
      ...
      try {
        // Try RPC first (vanilla equivalent)
        const safetyRpc = await supabase.rpc("safety_summary", { p_start: startDate, p_end: endDate });
      ...
      try {
        const scrapRpc = await supabase.rpc("scrap_top_end_summary", { p_start: startDate, p_end: endDate });
      ...
      } catch { /* defaults already 0 */ }

      /* Attendance — try RPC first */
      let attendanceResult: typeof attendance;
      let attendanceByShiftResult: typeof attendanceByShift;
      try {
        const attRpc = await supabase.rpc("attendance_summary", { p_start: startDate, p_end: endDate });
        if (!attRpc.error && attRpc.data && attRpc.data[0]) {
          ...
          attendanceResult = {
            ...
            // Cuti dibuang dari penyebut (bukan cuma pembilang), supaya cuti
            // tidak menurunkan rate kehadiran.
        ...
        if (attList.length > 0) {
          ...
          // Termasuk Cuti: cuti tetap masuk penyebut (tidak dibuang).
          const pctInclCuti = totOrang > 0 ? Math.min(100, Math.round((totHadir / totOrang) * 1000) / 10) : 0;
          // Tanpa Cuti: cuti dibuang dari penyebut, supaya tidak menurunkan rate.
      ...
      /* gsph_hourly per machine (harian mode only) */
      let hourlyResult: Record<string, { jam: number; gsph: number }[]> = {};
      if (periodMode === "harian") {
        try {
          const hourlyRes = await Promise.all(MACHINES.map((m) =>
            supabase.rpc("gsph_hourly", { p_mesin: m.key, p_start: rangeStartIso, p_end: rangeEndIso })
          ));
          MACHINES.forEach((m, idx) => {
      ...
      // Request ini sudah usang (periodMode/tanggal/dll berganti sebelum selesai) —
      // buang hasilnya, biarkan request yang lebih baru yang menentukan state akhir.
  ...
  /* fetchLineTrend + fetchMiniTrend + fetchProductivity after main data loads */
  useEffect(() => {
    if (loading) return;
    fetchLineTrend();
    fetchMiniTrend();
  ...
  useEffect(() => {
    ...
    if (hourlyCanvasRef.current) {
      ...
      if (periodMode === "harian") {
        // Use hourlyData from gsph_hourly RPC
        ...
        // Fallback labels if RPC didn't return data
        if (jamList.length === 0) {
        ...
        // Use lineTrend from fetchLineTrend
  ...
  }, [loading, vizMode, totals, ngRatePct, dtByCategoryMap, hourlyData, lineTrend, periodMode, theme]);
  ...
  useEffect(() => {
    ...
    // 1. Productivity Trend per Tanggal
    ...
    // 2. Productivity Akumulasi (Cumulative)
    ...
    // 3. Availability per Line (Target vs Actual)
    ...
    // 4. GSPH per Line (Target vs Actual)
    ...
    // 6. Downtime per Kategori (Pie/Doughnut)
    if (internalCategoryPieRef.current) {
      ...
      chartInstances.current._internalCategoryPieTotal = grandTotal as any;
    }

    // 7. Downtime per Kategori × Line (Stacked Bar)
  ...
  return (
    <HeaderNav activeTitle="Dashboard">
                ...
                {MACHINES.map((m) => {
                  ...
                  return (
                    <Card key={m.key} className="dash-panel card-glow-info">
                      ...
                      {rows.length === 0 ? (
                        <p className="dash-empty-msg">Tidak ada downtime.</p>
                      ) : (
                          ...
                          <table className="table-compact">
                            ...
                            <tbody>
                              {rows.map((r, i) => (
                                <tr key={i}>
                                  <td title={r.kategori}>
                                    <span className={`badge-kat badge-kat-${r.kategori.toLowerCase()}`}>
                                      {r.kategori}
                                    </span>
                                  </td>
      ...
      {/* Modal drill-down downtime — menggunakan Dialog shadcn */}
